<template>
    <div class="">
        ...
        <h2>Utilisation de Gp.Services.getConfig() <a
                href="http://jsfiddle.net/ignfgeoportail/0hsanbv2/embedded/result,js,html,css/">(jsfiddle)</a>.</h2>
                ...
                try {
                    Gp.Services.getConfig({
                        ...
                        onSuccess: function (result) {
                            ...
                            // droits
                            strResult += "<b>Ressources disponibles (" + Object.keys(lconf).length +
                                ")</b> : <br/>";
                            for (var lk in lconf) {
                                var lstr = "";
                                lstr += lconf[lk].title + ' (' + lconf[lk].name + ') - service : ' +
                                    lconf[lk].serviceParams.id;
                                strResult += "* " + lstr + "<br/>";
                            }
                            // WMTS layer conf
                            strResult +=
                                "<br/><b>Paramètres de la couche WMTS ORTHOIMAGERY.ORTHOPHOTOS</b> :</br>";
                            var lortho = lconf["ORTHOIMAGERY.ORTHOPHOTOS$GEOPORTAIL:OGC:WMTS"];
                            if (lortho) {
                                strResult += "* server URL : " + lortho.serviceParams.serverUrl[
                                    apiKey] + "<br/>";
                                ...
                                strResult += "* tms : " + lortho.wmtsOptions.tileMatrixSetLink +
                                    "</br>...";
                            }
                            // TMS..
